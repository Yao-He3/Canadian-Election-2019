...
\newpage
# Model/Methodology
We will be using a logistic regression model by the glm function in R to model the proportion of voters who will vote for Justin Trudeau with factors of age, sex, province, and education. And we will use backward elimination using AIC to get an alternative model. We will then compare the results we get from the two models. The main reason for choosing the logistic regression is that the dependent variable that we care about is the probability of voting for Justin Trudeau (vote_Trudeau). It is a binary variable. We want to make a prediction of the probability of Trudeau winning the election and it makes no sense if the predicted value is greater than 1 or less than 0. \newline
...
## Model Setup
This is the full logistic regression model we are using: \newline
Voting for Trudeau: \newline
...
The full model is comprehensive given the data that I obtained for this study. However, one drawback related to that is the occurrence of overfitting. \newline
...
## Post-Stratification 
In the above section, we created two models based on the survey data demographics and its stated voting choice. In this section, we will perform post-stratification analyses, which "incorporating population distributions of variables into survey estimates" (Little, 1993) for Trudeau's model to estimate the proportion of voters who will vote for him. \newline
In the analyses, we will group similar units together in order to reduce the variance. The variables we are using to construct the groups are: age, sex, province, and education. \newline
We will then weigh each proportion estimate (within each bin) by the respective population size of that bin and sum those values and divide that by the entire population size. \newline
